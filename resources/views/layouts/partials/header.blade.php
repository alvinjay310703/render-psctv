<header id="mainHeader" 
    ...
    <div class="flex items-center space-x-3">
        ...
        <button onclick="toggleSidebar()"
            class="hidden md:flex items-center justify-center w-10 h-10 bg-gradient-to-r from-gray-800 to-gray-700 text-white rounded-xl hover:from-gray-700 hover:to-gray-600 transition-all duration-200 shadow-lg hover:shadow-xl">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
            </svg>
        </button>
        ...
        <div class="ml-2 md:ml-6">
            <h1 class="text-xl md:text-2xl font-bold text-gray-900 tracking-tight">@yield('title')</h1>
            @hasSection('breadcrumbs')
                <nav class="hidden md:flex items-center text-sm text-gray-500 mt-1" aria-label="Breadcrumb">
    ...
    <div class="ml-auto flex items-center space-x-3 md:space-x-6">
        ...
        <div x-show="searchOpen" x-transition
            ...
            <div class="relative">
                ...
                <!-- Close mobile search -->
                <button @click="searchOpen = false" class="absolute right-12 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 md:hidden">
            ...
            <div x-show="open && results.length > 0" x-transition
                class="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-xl z-50 max-h-64 overflow-y-auto">
                <template x-for="(item, index) in results" :key="item.type + item.id">
                    <a :href="item.url"
                       ...
                       class="flex items-center px-4 py-3 text-sm text-gray-700 transition cursor-pointer">
            ...
            <div x-show="open && results.length === 0 && query.length > 1"
                class="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-xl p-4 text-gray-500 text-sm">
                No results found.
            </div>
        </div>

        <!-- Header Icons (Notifications, Status, User) -->
        <div class="flex items-center space-x-2 md:space-x-4">
            <!-- Notifications -->
            <div class="relative" x-data="{ open: false }">
                <button @click="open = !open" class="relative flex items-center justify-center w-10 h-10 bg-gray-50 hover:bg-gray-100 text-gray-600 hover:text-gray-800 rounded-xl transition-all duration-200 shadow-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div x-show="open" @click.away="open = false" x-transition.opacity
                    class="absolute right-0 mt-2 w-80 max-w-[90vw] bg-white shadow-xl rounded-xl border border-gray-200 z-50 overflow-hidden transition-all duration-200">
            ...
            <div class="relative" x-data="{ open: false }">
                <button @click="open = !open"
                    class="flex items-center justify-center w-10 h-10 bg-gray-50 hover:bg-gray-100 text-gray-600 hover:text-gray-800 rounded-xl transition-all duration-200 shadow-sm">
                ...
                <!-- Dropdown -->
                <div x-show="open" @click.away="open = false" x-transition.opacity
                    class="absolute right-0 mt-2 w-56 max-w-[80vw] bg-white border border-gray-200 rounded-xl shadow-xl z-50 overflow-hidden">
    ...
    <!-- Mobile Menu Panel -->
    <div x-show="mobileMenuOpen" x-transition
        class="fixed top-0 left-0 h-full w-80 max-w-[85vw] bg-white shadow-xl z-50 transform transition-transform duration-300"
